...
function inject (bot, { version }) {
  ...
  function updateBlock (point, type) {
    ...
    if (bot.majorVersion === '1.13') {
      column.setBlockStateId(posInChunk(point), type)
    } else {
      column.setBlockType(posInChunk(point), type >> 4)
      column.setBlockData(posInChunk(point), type & 0x0f)
    }
